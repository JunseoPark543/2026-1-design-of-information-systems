...
export function buildAdminInsights(metrics: AdminMenuMetric[], requests: MenuRequest[]): AdminInsight[] {
  ...
  metrics.forEach((metric) => {
    ...
    const menuName = withTopicParticle(metric.name);

    if (metric.sold_count >= highOrderThreshold && rating > 0 && rating < 4) {
      insights.push({
        title: "개선 필요",
        content: `${menuName} 주문량이 많지만 평균 평점이 낮아 맛, 가격, 구성 점검이 필요합니다.`,
        type: "improve",
    ...
    if (metric.sold_count > 0 && metric.sold_count < highOrderThreshold * 0.35 && rating >= 4.3) {
      insights.push({
        title: "홍보 강화 필요",
        content: `${menuName} 만족도가 높지만 주문량이 적어 추천 영역과 프로모션 노출을 늘릴 가치가 있습니다.`,
        type: "promote",
        menuName: metric.name,
      });
    }
    ...
    if (metric.sold_count >= highOrderThreshold && rating >= 4) {
      insights.push({
        title: "대표 메뉴 유지/홍보",
        content: `${menuName} 주문량과 평점이 모두 높아 대표 메뉴로 유지하고 세트 프로모션을 검토할 수 있습니다.`,
        type: "maintain",
        menuName: metric.name,
      });
    }
