...
4. `supabase/migrations/004_profile_signup_and_admin_filter.sql`
5. `supabase/seed.sql`
...
이미 `001_initial_schema.sql`과 `seed.sql`을 실행했다면, 관리자 회원/주문 조회와 가입일/관리자 제외 집계를 위해 `002_admin_profile_summary.sql`, `003_admin_order_summary.sql`, `004_profile_signup_and_admin_filter.sql`을 추가로 실행하면 됩니다. `Could not find the table public.admin_profile_summary in the schema cache` 오류도 이 migration을 실행하면 해결됩니다.
...
6. 소비자 전화번호+비밀번호 로그인을 바로 시연하려면 Supabase Auth의 Email confirmation을 꺼둡니다. 화면에는 이메일을 입력하지 않지만 내부 인증 ID를 만들기 위해 전화번호 기반 가상 이메일을 사용합니다.
...
- 소비자 회원가입은 이메일 입력 없이 전화번호와 비밀번호로 진행합니다. 내부적으로는 전화번호 기반 인증 ID를 만들어 Supabase Auth에 연결하므로 로그아웃 후에도 같은 전화번호와 비밀번호로 다시 로그인할 수 있습니다.
- 회원가입 시 `profiles`에 가입 연도, 월, 일을 함께 저장하며, 관리자 대시보드 회원 표에서 가입일을 확인할 수 있습니다.
- 별점 작성은 주문 내역 페이지에서 주문한 메뉴별로 가능하고, 메뉴 평균 평점에 반영됩니다.
- 메뉴 요청은 회원정보가 저장된 소비자만 가능하며, 같은 요청 메뉴는 요청 횟수가 증가합니다.
- 관리자 메뉴 CRUD의 데모 세션 생성을 위해 Supabase Dashboard의 Authentication 설정에서 Anonymous sign-ins를 켜두면 편합니다. 운영 버전에서는 server-side admin API로 교체하는 것이 좋습니다.
