    ...
    <PageShell className="py-10 lg:py-14">
      <section className="grid min-h-[calc(100vh-180px)] items-center gap-10 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600">주문, 별점, 요청 메뉴 데이터를 모아 소비자에게는 맞춤 메뉴를 추천하고, 점주에게는 매출과 메뉴 개선 인사이트를 제공하는 정보시스템입니다.</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <LinkButton href="/signup">소비자 시작하기<ArrowRight className="h-4 w-4" /></LinkButton>
            <LinkButton href="/menus" variant="secondary">메뉴 둘러보기</LinkButton>
          </div>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
          <div className="grid gap-4">
            {[
              { icon: ClipboardList, title: "데이터 수집", text: "주문, 별점, 요청 메뉴를 Supabase에 저장" },
              { icon: Sparkles, title: "맞춤 추천", text: "주문 이력과 별점, 요청 메뉴 기반 rule-based 추천" },
              { icon: BarChart3, title: "매출 전략", text: "인기 메뉴, 개선 메뉴, 신메뉴 후보를 관리자 화면에서 확인" },
